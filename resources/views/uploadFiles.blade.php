...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Files Page - {{$user->name}}</title>
  ...
  <body>
    <script>
       function getFileName(){
        ...
        fileName.value = myFile.files.item(0).name;
      }
    </script>
    <header>
      <div class="container">
      ...
      <div class="col-1">
        ...
        <div class="col-10">
          <div class="file-sources">
            <form action="/importFile" name = "form" method="POST" enctype="multipart/form-data"  value="file">
              @csrf
              <div class="upload_container">
                <input type="file" id="myFile" name="file" value="file" required="true" multiple onchange="getFileName()">
                <input type="hidden" id="fileName" name="fileName" value="fileName" required="true">
                <button type="submit" value="Send">Upload</button>
              </div>
            </form>
            <form action="/searchFile" name = "form" method="POST" enctype="multipart/form-data"  value="file">
              @csrf
              <div class="upload_container">
                <input type="text"  required="true" name = "fileSearch" placeholder="searchFile"></input>
            ...
            {{$downloadLinks->links()}}
              @foreach ($downloadLinks as $downloadLink)
                <div class="x">
                <a href="https://pixeldrain.com/api/file/{{$downloadLink->url_download}}?download=" class="downloadLinks"><i class="far fa-file"></i></a>
                <span>{{$downloadLink->fileName}}</span>
              </div>
              @endforeach
            </div>
          </div>
